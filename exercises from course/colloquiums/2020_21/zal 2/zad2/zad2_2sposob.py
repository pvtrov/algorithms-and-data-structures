...
"""
taktuje ta tablice jako wierzcholki grafu skierowanego i wykonuje sortowanie topologiczne
"""
...
def make_graph(L, M):
    graph = [[0 for _ in range(len(L))] for _ in range(len(L))]
    out_cycle = [False for _ in range(len(L))]

    for i in range(len(L)):
        for j in range(len(L)):
            if i != j:
                if L[i]%M == L[j]//M and out_cycle[j] is not True:
                    graph[i][j] = 1
                    graph[j][i] = 1
...
def order(L, K):
    M = 10**K
    directed_graph = make_graph(L, M)
    new_L = euler(directed_graph)
